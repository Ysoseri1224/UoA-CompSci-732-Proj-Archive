...
export function transition(ctx: GameContext, event: GameEvent): TransitionResult {
  ...
  switch (event.type) {
    ...
    case 'SKILL_CHANGE_COLOR':
      if (!canUseSkill(phase, ctx.roundState.skills.energy))
        return err(ctx, `Cannot use skill in phase ${phase}: not enough energy`);
    ...
    case 'SKILL_SHIELD':
      if (!canUseShield(phase, ctx.roundState.skills.energy, ctx.roundState.skills.shield))
