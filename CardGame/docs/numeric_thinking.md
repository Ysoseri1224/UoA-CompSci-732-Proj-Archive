...
> 从零到一的完整数值设计过程。Player HP=20，专精属性，10 个 buff 池。
...
核心原则：伤害 buff 分稳档（~15-22% DPS 增益）和搏档（~8-15% 或生存向），工具 buff 唯一选择。

### 稳档

| # | Buff | 数值 | DPS 增益 | 说明 |
|---|---|---|---|---|
| 1 | ELEMENT_CHIPS | 专精属性牌每张 +5 | +17% | 基于第一层专精选择 |
| 2 | 分层底分 | 常见+10 / 稀有+20 / 史诗+35 | +19% | 牌型越高级底分越高 |
| 3 | 分层倍率 | 常见+0 / 稀有+2 / 史诗+3 | +16% | 牌型越高级倍率越高 |

### 搏档

| # | Buff | 数值 | 增益 | 说明 |
|---|---|---|---|---|
| 4 | 固伤 | 每回合 +20 伤害 | +14% DPS | 无视牌型，稳定输出 |
| 5 | 攻击回血 | 每回合 +2 HP | 生存向 | 有效 HP 约 +8/层 |
| 6 | ELEMENT_CHIP_MULT | 专精属性 ×1.5 | +12% | 单牌点数倍增 |
| 7 | HP +5 | 最大生命值 +5 | 生存向 | 全层胜率上一档 |

### 工具（唯一选择）

| # | Buff | 效果 |
|---|---|---|
| 8 | Shuffle +1 | 每回合换牌 3 次 |
| 9 | 充能 +1 | 技能池上限 +1 |
| 10 | 手牌 +1 | 手牌上限 8 张 |

工具 buff 选过一次即从池子移除。稳档和搏档可重复叠加。
...
### 7.1 ALL_CHIPS_BONUS 被砍 → 替换为 ELEMENT_CHIPS
...
**决策**：砍掉，改为 `ELEMENT_CHIPS +5`（仅专精属性牌生效）。增益从 +20% 降到 +17%，但保证了属性专精的独占价值。玩家选择了火系专精 → 火牌每张多 5 点 → 更有动力去凑火系牌型，形成闭环。

---

### 7.2 单牌型 Buff 被放弃 → 改为全牌型分层 Buff
...
**衍生设计**：
- 分层底分 buff：常见+10 / 稀有+20 / 史诗+35 chip。实测增益 +19%，落在稳档区间。
- 分层倍率 buff：常见+0 / 稀有+2 / 史诗+3 mult。实测增益 +16%，落在稳档区间。
...
**决策**：改为固值回血——每回合攻击命中后回复固定 2 HP。每层约恢复 8HP，相当于有效 HP 增加约 40%。数值可控，不会溢出。
...
### 7.8 工具 Buff 设定为"唯一选择"

**原始方案**：工具 buff（Shuffle+1、充能+1、手牌+1）可重复叠加。

**问题**：Shuffle+1 叠加 2 次 = 每回合换牌 4 次 → 手牌几乎是"想换什么换什么"，牌型概率完全失控。手牌+1 叠加 2 次 = 9 张手牌 → 最优出牌的计算复杂度暴增（9 选 N 的组合数），且 DPS 曲线不可预测。

**决策**：三个工具 buff 各只能选一次，选了从池子移除。保证了这些"改变游戏规则"的能力不会叠加到失控。稳档和搏档 buff 仍可重复叠加——因为它们只是数值增减，不改变底层机制。
